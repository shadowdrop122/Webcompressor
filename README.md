...
3. 运行安装程序，记住安装路径（默认 `C:\Program Files\Java\jdk-17`）
4. 验证安装：
   ```
   打开命令提示符，输入：
   java -version
...
1. 访问 [JetBrains 官网](https://www.jetbrains.com/idea/download/) 下载 Community（免费版）
2. 运行安装程序，完成安装
3. 首次启动时配置 JDK：
   - 点击 `File` → `Project Structure` → `Platform Settings` → `SDKs`
   - 点击 `+` → `Add JDK`
   - 选择 JDK 17 的安装路径
...
|--------|-----|
| Name | WebCompressor |
| Main class | `compressor.gui.App` |
| VM options | `--module-path D:\javafx-sdk-17\lib --add-modules javafx.controls,javafx.fxml` |
...
1. 在 IDEA 中打开 `src/main/java/compressor/gui/App.java`
2. 点击编辑器左侧的绿色运行按钮，或使用快捷键 `Shift + F10`
...
# 编译并运行（需要配置 JavaFX SDK）
mvn clean compile

# 使用 Maven 运行（自动下载依赖）
mvn javafx:run -Djava.module.path=D:\javafx-sdk-17\lib
```
...
classDiagram
    ...
    class HuffmanCompressor {
        -codeTable: Map~Integer, String~
        -reverseCodeTable: Map~String, Integer~
        +compress(byte[]) byte[]
    ...
    ICompressor <|.. AbstractCompressor : implements
    AbstractCompressor <|-- HuffmanCompressor : extends
    AbstractCompressor <|-- LZ77Compressor : extends
    AbstractCompressor <|-- BrotliCompressor : extends
    AbstractCompressor <|-- PoolingImageCompressor : extends
    AbstractCompressor <|-- LZWImageCompressor : extends
    CompressorFactory ..> HuffmanCompressor : creates
    CompressorFactory ..> LZ77Compressor : creates
    CompressorFactory ..> BrotliCompressor : creates
    CompressorFactory ..> PoolingImageCompressor : creates
    CompressorFactory ..> LZWImageCompressor : creates
    ResourceDispatcher ..> ICompressor : uses
```
...
flowchart TD
    ...
    COMPRESS -->|Huffman| HF1[统计字符频率]
    HF1 --> HF2[构建霍夫曼树]
    HF2 --> HF3[生成编码表]
    HF3 --> HF4[变长编码输出]

    COMPRESS -->|LZ77| LZ1[滑动窗口扫描]
    LZ1 --> LZ2[查找最长匹配]
    LZ2 --> LZ3[输出&lt;偏移,长度&gt;引用]

    COMPRESS -->|Brotli| BR1[使用内置字典]
    BR1 --> BR2[DEFLATE压缩]
    BR2 --> BR3[输出压缩数据]

    COMPRESS -->|PoolingImage| PI1[图像解码]
    PI1 --> PI2[均值池化降采样]
    PI2 --> PI3[有损压缩输出]
...
flowchart TD
    ...
    MATCH -->|.html .htm .css .js<br/>.txt .xml .json| TEXT[文本文件]
    MATCH -->|.jpg .jpeg .png<br/>.gif .webp .bmp| IMAGE[图片文件]
    MATCH -->|.woff .woff2 .ttf<br/>.eot .otf .ico| FONT[字体/二进制]
    ...
    ALGO1 --> RESULT1["压缩率: 60-80%<br/>特点: 内置优化字典"]
    ALGO2 --> RESULT2["压缩率: 50-90%<br/>特点: 有损,可调质量"]
    ALGO3 --> RESULT3["压缩率: 40-70%<br/>特点: 无损,基于重复"]
    ALGO4 --> RESULT4["压缩率: 60-80%<br/>特点: 通用高效"]
...
flowchart TD
    ...
    subgraph DICT["内置字典"]
        B1["包含常见的:<br/>HTML标签<br/>CSS属性<br/>JavaScript关键字<br/>域名等"]
    end
